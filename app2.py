...
# Initialize Groq
GROQ_API_KEY = "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"
llm = ChatGroq(
...
# Neo4j Configuration
NEO4J_URI = "neo4j+s://f8f2e130.databases.neo4j.io"
NEO4J_USER = "neo4j"
NEO4J_PASSWORD = "vT2_arob6eOaCBBJ-S2RcmAxtiD-_6Fh_SL47shKy0E"
...
class KnowledgeGraphRAG:
    ...
    def create_3d_graph(self, node_size=8, node_color='#00ff00', edge_width=1, edge_color='#888', spring_k=0.1):
        ...
        pos = nx.spring_layout(G, dim=3, k=spring_k)  # Use spring_k for spacing
...
def main():
    ...
    with st.sidebar:
        ...
        node_size = st.slider("Node Size", min_value=5, max_value=20, value=8, step=1)
        node_color = st.color_picker("Node Color", value="#00ff00")
        edge_width = st.slider("Edge Width", min_value=1, max_value=10, value=1, step=1)
        edge_color = st.color_picker("Edge Color", value="#888888")
        spring_k = st.slider("Node Spacing (Spring Constant)", min_value=0.1, max_value=1.0, value=0.1, step=0.1)
    
    # Main content area (unchanged until graph visualization)
    ...
    # 3D Graph Visualization with settings
